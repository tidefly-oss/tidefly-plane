...
func NewAuthService(db *gorm.DB, jwtSvc *Service, tokenStore *TokenStore) *AuthService {
	return &AuthService{
		db:         db,
		jwt:        jwtSvc,
		tokenStore: tokenStore,
...
func (s *AuthService) Login(ctx context.Context, input LoginInput) (*models.User, *TokenPair, error) {
	...
	if err := s.db.Where("email = ?", input.Email).First(&user).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			_ = VerifyPassword(
				input.Password,
				"$argon2id$v=19$m=65536,t=3,p=2$dummysalt123456$dummyhash1234567890123456789012",
			)
...
func (s *AuthService) GetUser(_ context.Context, userID string) (*models.User, error) {
	var user models.User
	if err := s.db.First(&user, "id = ?", userID).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, ErrUserNotFound
