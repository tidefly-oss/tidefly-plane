...
func (b *Bitbucket) GetInfo() ProviderInfo {
	return ProviderInfo{Type: ProviderBitbucket, DisplayName: "Bitbucket", BaseURL: "https://bitbucket.org"}
}
...
func (b *Bitbucket) ListRepositories(ctx context.Context) ([]Repository, error) {
	...
	repos := make([]Repository, len(page.Values))
...
func (b *Bitbucket) GetRepository(ctx context.Context, owner, name string) (*Repository, error) {
	req, err := b.newRequest(
		ctx,
		http.MethodGet,
...
func (b *Bitbucket) ListBranches(ctx context.Context, owner, name string) ([]Branch, error) {
	req, err := b.newRequest(
		ctx,
		http.MethodGet,
	...
	branches := make([]Branch, len(page.Values))
	for i, br := range page.Values {
		branches[i] = Branch{Name: br.Name, CommitSHA: br.Target.Hash}
	}
	return branches, nil
}
