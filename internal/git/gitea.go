...
func (g *GiteaForgejo) GetInfo() ProviderInfo {
	displayName := "Gitea"
	if g.isForgejo {
		displayName = "Forgejo"
	}
	return ProviderInfo{Type: ProviderGiteaForgejo, DisplayName: displayName, BaseURL: g.baseURL}
}
...
func (g *GiteaForgejo) ListRepositories(ctx context.Context) ([]Repository, error) {
	...
	repos := make([]Repository, len(result.Data))
...
func (g *GiteaForgejo) GetRepository(ctx context.Context, owner, name string) (*Repository, error) {
...
func (g *GiteaForgejo) ListBranches(ctx context.Context, owner, name string) ([]Branch, error) {
	...
	for i, b := range raw {
		branches[i] = Branch{Name: b.Name, CommitSHA: b.Commit.ID}
	}
	return branches, nil
}
