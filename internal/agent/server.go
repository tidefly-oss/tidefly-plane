...
// Registry returns the worker registry (used by other manifest to send commands).
...
func (s *Server) Ping(_ context.Context, req *proto.PingRequest) (*proto.PingResponse, error) {
...
func (s *Server) handleHello(stream proto.AgentService_ConnectServer, worker *models.WorkerNode, ip string) error {
	msg, err := stream.Recv()
	if err != nil {
		return status.Error(codes.Internal, "failed to receive hello")
	}

	hello, ok := msg.Payload.(*proto.AgentMessage_Hello)
	if !ok {
	...
	return stream.Send(
		&proto.PlaneMessage{
			CommandId: "hello-ack",
			WorkerId:  worker.ID,
			Payload: &proto.PlaneMessage_Ack{
				Ack: &proto.PlaneAck{Accepted: true},
			},
		},
	)
}
