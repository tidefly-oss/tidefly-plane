...
type WorkerConn struct {
	...
	logs    map[string]chan *proto.ContainerLogsResult // commandID → log channel
}
...
func newWorkerConn(workerID string, stream proto.AgentService_ConnectServer) *WorkerConn {
	return &WorkerConn{
		workerID: workerID,
		stream:   stream,
		done:     make(chan struct{}),
		pending:  make(map[string]*pendingCmd),
		logs:     make(map[string]chan *proto.ContainerLogsResult),
	}
}
...
func (c *WorkerConn) Send(msg *proto.PlaneMessage) (any, error) {
	p := &pendingCmd{
...
func (c *WorkerConn) SendNoWait(msg *proto.PlaneMessage) error {
	return c.stream.Send(msg)
}

// StreamLogs opens a log channel for a command and returns it.
// Caller must close the returned channel when done.
func (c *WorkerConn) StreamLogs(commandID string) chan *proto.ContainerLogsResult {
	ch := make(chan *proto.ContainerLogsResult, 100)
	c.mu.Lock()
	c.logs[commandID] = ch
	c.mu.Unlock()
	return ch
...
func (c *WorkerConn) pushLog(log *proto.ContainerLogsResult) {
	c.mu.Lock()
...
func (r *Registry) Register(workerID string, stream proto.AgentService_ConnectServer) *WorkerConn {
