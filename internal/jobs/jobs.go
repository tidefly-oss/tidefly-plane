...
func (s *Server) Run(ctx context.Context) error {
	...
	mux.HandleFunc(queue.TaskServiceCleanup, s.svcHandler.HandleServiceCleanup)
	...
	mux.HandleFunc(queue.TaskServiceDeploy, s.svcHandler.HandleServiceDeploy)
	mux.HandleFunc(queue.TaskServiceRedeploy, s.svcHandler.HandleServiceRedeploy)
	mux.HandleFunc(queue.TaskServiceUpdate, s.svcHandler.HandleServiceUpdate)
	mux.HandleFunc(queue.TaskServiceDelete, s.svcHandler.HandleServiceDelete)
	...
	mux.HandleFunc(queue.TaskServiceHeal, s.svcHandler.HandleServiceHeal)
...
func (s *Server) registerSchedules() error {
	...
	if _, err := s.scheduler.Register(s.cfg.CleanupCron, cleanupTask,
		asynq.Queue("low"), asynq.MaxRetry(2), asynq.Timeout(10*time.Minute)); err != nil {
	...
	if _, err := s.scheduler.Register(s.cfg.HealthCheckCron, healthTask,
		asynq.Queue("critical"), asynq.MaxRetry(1), asynq.Timeout(2*time.Minute)); err != nil {
		return fmt.Errorf("register healthcheck: %w", err)
	}

	retentionTask, err := newTask(TaskLogsRetention, map[string]any{
	...
	if _, err := s.scheduler.Register(s.cfg.LogRetentionCron, retentionTask,
		asynq.Queue("low"), asynq.MaxRetry(1), asynq.Timeout(5*time.Minute)); err != nil {
	...
	if _, err := s.scheduler.Register("@every 15s", metricsTask,
		asynq.Queue("critical"), asynq.MaxRetry(0), asynq.Timeout(10*time.Second)); err != nil {
	...
	if _, err := s.scheduler.Register("@every 30s", serviceHealthTask,
		asynq.Queue("critical"), asynq.MaxRetry(0), asynq.Timeout(30*time.Second)); err != nil {
	...
	if _, err := s.scheduler.Register("@every 30s", autoscaleTask,
		asynq.Queue("critical"), asynq.MaxRetry(0), asynq.Timeout(25*time.Second)); err != nil {
	...
	if _, err := s.scheduler.Register("@every 6h", updateCheckTask,
		asynq.Queue("low"), asynq.MaxRetry(1), asynq.Timeout(5*time.Minute)); err != nil {
