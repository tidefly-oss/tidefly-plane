...
func (h *ServiceJobHandler) HandleServiceDeploy(ctx context.Context, t *asynq.Task) error {
	var p queue.ServiceDeployPayload
	...
	result, err := conv.Convert(ctx, converter.ConvertInput{
		ManifestJSON: p.Input.ManifestJSON,
		Image:        p.Input.Image,
		ComposeYAML:  p.Input.ComposeYAML,
		Dockerfile:   p.Input.Dockerfile,
		GitURL:       p.Input.GitURL,
		Name:         p.Input.Name,
		StackName:    p.Input.StackName,
		ProjectID:    p.Input.ProjectID,
		Domain:       p.Input.Domain,
		Port:         p.Input.Port,
		Expose:       p.Input.Expose,
		Branch:       p.Input.Branch,
		GitToken:     p.GitToken,
		Replicas:     p.Input.Replicas,
		Strategy:     p.Input.Strategy,
	})
	if err != nil {
...
func (h *ServiceJobHandler) HandleServiceRedeploy(ctx context.Context, t *asynq.Task) error {
	var p queue.ServiceRedeployPayload
	if err := json.Unmarshal(t.Payload(), &p); err != nil {
...
func (h *ServiceJobHandler) HandleServiceUpdate(ctx context.Context, t *asynq.Task) error {
	var p queue.ServiceUpdatePayload
	if err := json.Unmarshal(t.Payload(), &p); err != nil {
...
func (h *ServiceJobHandler) HandleServiceDelete(ctx context.Context, t *asynq.Task) error {
	var p queue.ServiceDeletePayload
	if err := json.Unmarshal(t.Payload(), &p); err != nil {
	...
	if len(images) > 0 || len(volumes) > 0 {
		if err := queue.EnqueueServiceCleanup(h.client, svc.Name, images, volumes); err != nil {
