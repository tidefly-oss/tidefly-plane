...
// DeployCompose und DeleteStack bleiben auf rohem Echo.
...
func (h *Handler) DeployCompose(ctx context.Context, input *DeployComposeInput) (*DeployComposeOutput, error) {
	...
	if err := h.db.First(&project, "id = ?", input.Body.ProjectID).Error; err != nil {
...
// BuildAndDeployEcho Legacy Echo-Handler für BuildAndDeploy (SSE) — registrierung direkt auf Echo
func (h *Handler) BuildAndDeployEcho(c *echo.Context) error {
	return h.BuildAndDeploy(c)
}
